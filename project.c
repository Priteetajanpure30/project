//Projects/Customised_Dynamic_File_System.c
#include<stdio.h>
#include<stdlib.h>
#include<string.h>
#include<unistd.h>
...
int main()
{
	...
	InitialiseSuperBlock();					//done
	CreateDILB();							//done

	while(1)
	{
		fflush(stdin);					//input buffer clean kra fflush -> clean,stdin-> input buffer(UFDT[0])
		strcpy(str,"");
		printf("\nPritee VFS : > ");
		...
		if(count == 1)
		{
			...
			else if(strcmp(command[0],"exit") == 0)						//option deletelast
			{
